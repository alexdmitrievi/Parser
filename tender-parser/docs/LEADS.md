...
> **Внимание: сейчас GitHub Actions не видит ни один workflow проекта.**
>
> GitHub читает workflow только из `.github/workflows/` в **корне
> репозитория**. В репозитории `alexdmitrievi/Parser` корень — это уровень
> выше, а весь проект лежит в подкаталоге `tender-parser/`, поэтому файлы
> находятся по пути `tender-parser/.github/workflows/` и не подхватываются.
>
> Так стало после коммита `ffcc846` (2026-05-06, «production hardening v4»),
> который перенёс проект в подкаталог и удалил корневой `.github/`. Последний
> прогон любого workflow — в тот же день; с тех пор не выполнялось ни одного
> (всего в истории 1768 прогонов, все до 2026-05-06).
>
> Это касается **всех** workflow, а не только `leads`: парсинг тендеров,
> грантов, уведомления, бэкапы и тесты по расписанию тоже не запускаются.
> `parse-leads.yml` и `test-leads.yml` положены рядом с остальными — для
> единообразия и чтобы они заработали сразу, как только проблема будет решена.
>
> Починка — вынести `.github/` в корень репозитория (со `working-directory:
> tender-parser` в шагах) либо сделать `tender-parser` корнем репозитория.
> Это не сделано намеренно: восстановление корневого `.github/` разом оживит
> все спящие парсеры и рассылку уведомлений реальным пользователям бота, а
> это решение владельца проекта, а не побочный эффект добавления домена.
>
> Пока вопрос не решён, домен `leads` запускается вручную или через systemd
> (ниже) — на работоспособность самого модуля это не влияет.
...
Тесты лежат в `engine/tests/leads/`, а не в `tests/`, потому что существующий
CI ставит только `requirements.txt` (без bs4 и PyYAML) и гоняет `pytest tests/`.
У домена свой workflow — `.github/workflows/test-leads.yml`, так что добавление
`leads` не может уронить проверки `tenders` и `funding`.

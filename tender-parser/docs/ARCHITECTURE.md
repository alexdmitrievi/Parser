...
зарубежных адресов, эти площадки недоступны в принципе. Варианты решения —
российский VPS (заготовки systemd-юнитов лежат в `deploy/systemd/`),
коммерческий scraping-API (в секретах репозитория есть неиспользуемый
`SCRAPING_BEE_API_KEY`), гибрид или отказ от недоступных площадок — требуют
решения владельца, а не агента.

"""Парсер Магнит (tender.magnit.ru).

Корпоративный портал закупок сети Магнит (223-ФЗ и коммерческие).
URL: tender.magnit.ru
"""
...
class MagnitScraper(BaseScraper):
    """Парсер tender.magnit.ru."""

    platform = "magnit"
    base_url = "https://tender.magnit.ru"
    min_delay = 3.0
    max_delay = 8.0

    SEARCH_URL = "https://tender.magnit.ru/tenders"
    API_URL = "https://tender.magnit.ru/api/tenders"
    ...
    def _fetch_api(self, query: str, page: int = 1, size: int = 20) -> list[dict]:
        params = {"keyword": query, "page": str(page), "pageSize": str(size), "status": "active"}
        try:
    ...
    def _fetch_html(self, query: str, page: int = 1) -> list[dict]:
        ...
        try:
            ...
            logger.warning(f"[Magnit] Fetch error: {e}")
            return []
        ...
        rows = soup.select(
            ...
            "div[class*='tender'], div[class*='purchase-item']"
        )
    ...
    def run(self, queries: list[str] | None = None, max_pages: int = 3, **kwargs) -> list[TenderCreate]:
        if queries is None:
            queries = [
                ...
                "продукты питания", "торговое оборудование",
                "IT", "транспортные услуги", "логистика",
                "охрана", "клининг", "холодильное оборудование",
            ]
        ...
        with self:
            for query in queries:
                logger.info(f"[Magnit] Searching: {query}")
        ...
        logger.info(f"[Magnit] Total: {len(all_tenders)} tenders")
        return all_tenders

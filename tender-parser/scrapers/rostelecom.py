"""Парсер Ростелеком (zakupki.rt.ru).

Корпоративный портал закупок Ростелекома (223-ФЗ).
URL: zakupki.rt.ru
"""
...
class RostelecomScraper(BaseScraper):
    """Парсер zakupki.rt.ru."""

    platform = "rostelecom"
    base_url = "https://zakupki.rt.ru"
    min_delay = 3.0
    max_delay = 8.0

    SEARCH_URL = "https://zakupki.rt.ru/purchases"
    API_URL = "https://zakupki.rt.ru/api/v1/purchases"
    ...
    def _fetch_html(self, query: str, page: int = 1) -> list[dict]:
        ...
        try:
            ...
            logger.warning(f"[Rostelecom] Fetch error: {e}")
            return []
        ...
        rows = soup.select(
            ...
            "div[class*='purchase'], div[class*='tender-item']"
        )
    ...
    def run(self, queries: list[str] | None = None, max_pages: int = 3, **kwargs) -> list[TenderCreate]:
        if queries is None:
            queries = [
                "строительство", "ремонт", "телекоммуникации",
                "IT оборудование", "сетевое оборудование",
                "поставка оборудования", "программное обеспечение",
                "транспортные услуги", "охрана", "клининг",
            ]
        ...
        with self:
            for query in queries:
                logger.info(f"[Rostelecom] Searching: {query}")
        ...
        logger.info(f"[Rostelecom] Total: {len(all_tenders)} tenders")
        return all_tenders

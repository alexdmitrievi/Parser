"""Парсер Норникель (tenders.nornickel.ru).

Корпоративный портал закупок Норникеля (223-ФЗ).
URL: tenders.nornickel.ru
"""
...
class NornickelScraper(BaseScraper):
    """Парсер tenders.nornickel.ru."""
    ...
    SEARCH_URL = "https://tenders.nornickel.ru/tenders"
    API_URL = "https://tenders.nornickel.ru/api/v1/tenders"
    ...
    def _fetch_api(self, query: str, page: int = 1, size: int = 20) -> list[dict]:
        params = {"query": query, "page": str(page), "pageSize": str(size), "status": "active"}
        try:
    ...
    def _fetch_html(self, query: str, page: int = 1) -> list[dict]:
        ...
        try:
            ...
            logger.warning(f"[Nornickel] Fetch error: {e}")
            return []
        ...
        rows = soup.select(
            "table tbody tr, "
            ".tender-item, .tender-row, .tender-card, "
        ...
        for row in rows:
            ...
            price_el = row.select_one(".price, .nmck, .start-price, [class*='price']")
            if price_el:
    ...
    def run(self, queries: list[str] | None = None, max_pages: int = 3, **kwargs) -> list[TenderCreate]:
        if queries is None:
            queries = [
                "строительство", "ремонт", "горное оборудование",
                "поставка оборудования", "металлоконструкции",
                "транспортные услуги", "IT", "проектирование",
                "охрана", "спецодежда", "химические реагенты",
            ]
        ...
        with self:
            for query in queries:
                logger.info(f"[Nornickel] Searching: {query}")
        ...
        logger.info(f"[Nornickel] Total: {len(all_tenders)} tenders")
        return all_tenders

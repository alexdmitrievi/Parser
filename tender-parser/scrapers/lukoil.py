"""Парсер Лукойл (tender.lukoil.ru).

Корпоративный портал закупок Лукойл (223-ФЗ).
URL: tender.lukoil.ru
"""
...
class LukoilScraper(BaseScraper):
    """Парсер tender.lukoil.ru."""
    ...
    SEARCH_URL = "https://tender.lukoil.ru/tenders"
    API_URL = "https://tender.lukoil.ru/api/tenders"
    ...
    def _fetch_api(self, query: str, page: int = 1, size: int = 20) -> list[dict]:
        params = {"search": query, "page": str(page - 1), "size": str(size)}
        try:
            resp = self.fetch(f"{self.API_URL}?{urlencode(params)}")
            data = resp.json()
            return data.get("content", data.get("items", data.get("tenders", [])))
        except Exception:
            return []
    ...
    def _fetch_html(self, query: str, page: int = 1) -> list[dict]:
        ...
        try:
            ...
            logger.warning(f"[Lukoil] Fetch error: {e}")
            return []
        ...
        for row in rows:
            item: dict = {}
            link = row.select_one("a[href*='tender'], a[href*='lot'], h2 a, h3 a, .subject a, td.name a")
            if not link:
                continue
    ...
    def _api_to_dict(self, tender: dict) -> Optional[dict]:
        title = (
            tender.get("tenderName") or tender.get("subject") or
            tender.get("name") or tender.get("title", "")
        )
        ...
        return {
            ...
            "region": tender.get("deliveryRegion") or tender.get("region"),
        }
    ...
    def run(self, queries: list[str] | None = None, max_pages: int = 3, **kwargs) -> list[TenderCreate]:
        if queries is None:
            queries = [
                "строительство", "ремонт", "бурение",
                "поставка оборудования", "нефтепромысловое",
                "транспортные услуги", "IT", "проектирование",
                "охрана", "монтаж трубопровода",
            ]
        ...
        with self:
            for query in queries:
                logger.info(f"[Lukoil] Searching: {query}")
        ...
        logger.info(f"[Lukoil] Total: {len(all_tenders)} tenders")
        return all_tenders
